...
export async function startDevServer(output: string): Promise<{ url: string; devServer: ChildProcessWithoutNullStreams }> {
    return await new Promise((resolve) => {
        const devServerProcess = spawn("pnpm", ["run", "dev"], { stdio: "pipe", shell: true, cwd: output });
